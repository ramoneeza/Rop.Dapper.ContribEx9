...
public static partial class DapperHelperExtend
{
    ...
    public static string SelectGetAllCache(Type type)
    {
        ...
        if (!GetQueries.TryGetValue(listoft.TypeHandle, out var sql))
        {
            var name = GetTableName(type);
    ...
    {
        if (!DeleteByKeyDic.TryGetValue(type.TypeHandle, out var sql))
        {
            var (key, _) = GetSingleKey(type);
            var name = GetTableName(type);
    ...
    {
        if (!DeleteByPartialKeyDic.TryGetValue(type.TypeHandle, out var sql))
        {
            var kd = GetPartialKeyDescription(type);
            if (kd == null) throw new ArgumentException($"{type} has not partial key");
            var name = kd.TableName;
            var key = kd.KeyName;
            sql = $"DELETE FROM {name} WHERE {key} = @id";
            DeleteByPartialKeyDic[type.TypeHandle] = sql;
        }
        return sql;
    }
    ...
    {
        if (ForeignDatabase.TryGetValue(type.TypeHandle, out var name)) return name;
        var foreigndatabaseAttrName = type.GetCustomAttribute<ForeignDatabaseAttribute>(false)?.Name??"";
        if (foreigndatabaseAttrName == "")
        {
    ...
    {
        if (SelectSlimDic.TryGetValue(type.TypeHandle, out var partialSelect)) return partialSelect;
    ...
    {
        if (GetPartialDic.TryGetValue(type.TypeHandle, out var querypartial)) return querypartial;
        var kd = GetPartialKeyDescription(type)??throw new ArgumentException($"{type} has not partial key");
        querypartial = $"SELECT * FROM {kd.TableName} where {kd.KeyName} = @id";
        GetSlimDic[type.TypeHandle] = querypartial;
        return querypartial;
    }
